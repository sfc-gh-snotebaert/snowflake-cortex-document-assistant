...
def create_prompt(user_question, document_content=""):
    """
    Create a prompt for the language model by combining the user question with parsed document content.
    Format the prompt according to the expected input format of the model.

    Args:
        user_question (str): The user's question to generate a prompt for.
        document_content (str): Content from parsed documents

    Returns:
        str: The generated prompt for the language model.
    """
    prompt_context = document_content
    ...
    prompt = f"""
        ...
        Review the following question and the provided documents to answer the question in a legally accurate manner. 
        ...
        Please provide a helpful and accurate response based on the documents provided.
        If the answer cannot be determined from the documents, just say "I don't have enough information to answer that question."
        [/INST]
        Answer:
    """
...
# Function to process files with document parsing only
def process_files_with_ai_parallel(file_records, client_name, upload_id):
    ...
    try:
        ...
        for i in range(0, total_files, batch_size):
            ...
            # Parse documents for the entire batch in parallel
            parse_futures = []
            for record in batch:
...
def render_chat_interface(client_name=None, upload_id=None):
    ...
    if question := st.chat_input("Ask about your documents..."):
        ...
        with st.chat_message("assistant", avatar=icons["assistant"]):
            ...
            # Get document content for the selected client/upload
            with st.spinner("Retrieving document content..."):
                document_content = get_document_content(client_name, upload_id)
            
            # Create prompt and generate response
            with st.spinner("Thinking..."):
                prompt = create_prompt(question_safe, document_content)
...
# Function to merge content from parsed documents with descriptors
def merge_content_with_descriptors(df, column_name='PARSED_DOCUMENT'):
    # Collect the data from Snowflake to the client
    collected_data = df.select(col(column_name)).collect()
    
    # Now merge the content with descriptors
    merged_content = ""
    for i, row in enumerate(collected_data, 1):
        content = row[column_name]
        if content and isinstance(content, str):
            # Extract content between the quotes after "content":
            match = re.search(r'"content":\s*"([^"]*)"', content)
            if match:
                # Add descriptor before the content
                merged_content += f"[Document_{i}/Begin]\n"
                # Add the content
                merged_content += match.group(1)
                # Add descriptor after the content
                merged_content += f"\n[Document_{i}/End]\n\n"
    
    return merged_content

# Get document content for a specific client and upload
def get_document_content(client_name, upload_id):
    """Get content from the parsed documents for a specific client upload"""
    content = ""
    
    if client_name and upload_id:
        try:
            # Create a dataframe of parsed documents for this upload
            df = session.table("CONTRACT_PDF_FILES").filter(
                (col("UPLOAD_ID") == upload_id)
            )
            
            # Merge the content with descriptors
            content = merge_content_with_descriptors(df)
            
            if not content:
                st.warning("No parsed content found for the selected documents.")
        except Exception as e:
            st.sidebar.error(f"Error retrieving document content: {str(e)}")
